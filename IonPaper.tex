...
The receiver front end uses two uBlox ANN-MB-00-00 multiband active GNSS antennas connected to the two receiver channels of an USRP B210. The antennas are separated by approximately $3.75$~in. ($0.095$~m), which is approximately one half wavelength at GPS L1. Since $\lambda = c/f_{L1}$ and $f_{L1}=1575.42$~MHz, the GPS L1 wavelength is approximately $0.190$~m, giving $\lambda/2 \approx 0.095$~m. In multi-antenna phased arrays, half-wavelength spacing is a vital constraint to prevent spatial aliasing. These ublox antennas also incorporate internal Low Noise Amplifiers (LNAs), which require a direct current (DC) bias voltage to operate. Thus, the signal path uses independent USB Bias Tee GPIO Labs modules, which inject the DC power into the radio frequency (RF) signal path to the active antennas. The RF outputs from the respective bias tees are routed directly into the Ettus Research USRP B210 via the physical ports labeled RF A RX2 and RF B RX2~\cite{ettusB210}. These physical ports are respectively mapped to Channel 0 and Channel 1.
...
\begin{figure}[h]
    ...
    \caption{Physical hardware setup including the USRP B210, dual ublox antennas on a metal plate, and GPIO Labs bias tees.}
...
Analyzing the primary/secondary-tracking data for PRNs 3, 12, and 31 demonstrates the impact of the jamming on the correlator outputs. Figures \ref{fig:prn3_inphase}, \ref{fig:prn12_inphase}, and \ref{fig:prn31_inphase} plot the Signal-to-Noise Ratio (VSM value). Across all three tracked satellites, as jammer power increases, Cn0 goes down. Simultaneously, the standard deviation of the in-phase increases (spiking from approximately $3^\circ$ to nearly $100^\circ$) as the jammer overwhelms the C/A code, destroying the correlation peak. A similar pattern occurs in the quadrature channel, though it is omitted in this paper for conciseness. This behavior makes logical sense, because as the CW interference power rises, the thermal noise floor is eclipsed, causing the Delay Lock Loop (DLL) and Phase Lock Loop (PLL) discriminators to fail.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{photos/Jamming Analysis for PRN 3.png}
    \caption{PRN 3 (overhead): Signal-to-Noise Ratio}
...
\begin{figure}[H]
    ...
    \caption{PRN 12 (near horizon): Signal-to-Noise Ratio}
...
\begin{figure}[H]
    ...
    \caption{PRN 31 (east): Signal-to-Noise Ratio}
    \label{fig:prn31_inphase}
\end{figure}

%In-Phase Standard Deviation, demonstrating degrading signal strength and spiking due to CW interference.

The inter-channel phase difference calculated via the primary/secondary correlators and its associated standard deviation are plotted in Figures \ref{fig:prn3_phasediff}, \ref{fig:prn12_phasediff}, and \ref{fig:prn31_phasediff}. 

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{photos/Phase Analysis for PRN 3.png}
    \caption{PRN 3 (overhead): Correlator-derived Inter-Channel Phase Difference and Phase Standard Deviation}
...
However, these post-correlation phase difference graphs do not accurately reflect the true Angle of Arrival of the jammer. Since primary/secondary correlators rely on mixing the incoming signal with a locally generated C/A code replica, the correlators are combining pure noise and interference over the PRN sequence rather than isolating the CW tone. Consequently, the standard deviation of the phase difference dramatically increases (exceeding $50^\circ$), and the raw phase differences show no correlation.
